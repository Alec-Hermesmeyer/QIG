...
-- 3. Create client_configurations table
...
-- 4. Create client_secrets table for encrypted sensitive data
...
-- Policy: Users can access their own profile and profiles in their organization
...
-- 9. Insert a default organization if none exists
INSERT INTO organizations (name, description) 
SELECT 'Default Organization', 'Default organization for initial setup'
WHERE NOT EXISTS (SELECT 1 FROM organizations WHERE name = 'Default Organization');

-- 10. Create function to get client configuration with secrets (for server-side use)
...
-- 11. Only insert default configuration if we have organizations and profiles
DO $$
DECLARE
    default_org_id UUID;
...
BEGIN
    ...
    -- Get any profile (create a dummy one if none exists)
    SELECT id INTO default_profile_id FROM profiles LIMIT 1;
    
    -- If no profiles exist, we'll skip the default config insertion
    -- It can be created later via the admin interface
    IF default_org_id IS NOT NULL AND default_profile_id IS NOT NULL THEN
