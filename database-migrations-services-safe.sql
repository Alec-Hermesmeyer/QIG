-- Safe Service Mapping Extensions for Multi-Service Architecture
...
-- 2. Create organization_services junction table if it doesn't exist
...
-- 3. Update client_configurations to include service reference if column doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'client_configurations' AND column_name = 'service_id'
    ) THEN
...
BEGIN
    ...
    DROP POLICY IF EXISTS "All users can read services" ON services;
    DROP POLICY IF EXISTS "Users can access their organization's services" ON organization_services;
...
-- 7. Create triggers for updated_at (only if function exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_column') THEN
        DROP TRIGGER IF EXISTS update_services_updated_at ON services;
        DROP TRIGGER IF EXISTS update_organization_services_updated_at ON organization_services;
...
-- 8. Insert the three main services (ON CONFLICT DO NOTHING)
...
-- 9. Create function to get organization services with configurations
...
-- 10. Create function to get client configuration for a specific service
...
-- 11. Create or replace view for easy service overview
DROP VIEW IF EXISTS organization_service_overview;
CREATE VIEW organization_service_overview AS
SELECT 
...
-- 12. Verify the setup
SELECT 'Services migration completed successfully!' as status;
